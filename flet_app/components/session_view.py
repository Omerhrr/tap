...
class SessionView(ft.Column):
    ...
    def _build_controls(self) -> list:
        ...
        app_bar = ft.Container(
            content=ft.Row(
                controls=[
                    ft.IconButton(icon="arrow_back", on_click=self._on_back_click),
                    ft.Text(f"Session: {state.session_code}", size=20, weight="bold", expand=True),
                    ft.IconButton(icon="qr_code", on_click=self._on_qr_click, tooltip="Show QR Code")
                ],
        ...
        participants_container = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("Participants", size=14, color="grey"),
        ...
        qr_dialog = ft.AlertDialog(
            ...
            content=ft.Column(
                controls=[
                    ...
                    ft.Container(
                        content=ft.Text(state.session_code, size=48, weight="bold", color="indigo"),
                        alignment="center",
                        padding=20
                    )
                ],
    ...
    def _show_add_item_dialog(self):
        ...
        amount_field = ft.TextField(label="Amount", keyboard_type="number")
        quantity_field = ft.TextField(label="Quantity", value="1", keyboard_type="number")

        add_dialog = ft.AlertDialog(
            title=ft.Text("Add Item"),
            content=ft.Column(
                controls=[description_field, amount_field, quantity_field],
                tight=True
            ),
            actions=[
    ...
    def _show_edit_item_dialog(self, item: dict):
        description_field = ft.TextField(label="Description", value=item.get('description', ''))
        amount_field = ft.TextField(label="Amount", value=str(item.get('amount', 0)), keyboard_type="number")
    ...
    async def _update_item(self, item_id: int, description: str, amount: float):
        try:
            await api_client.update_item(item_id, {"description": description, "amount": amount})
            self._close_dialog()
        except Exception as ex:
